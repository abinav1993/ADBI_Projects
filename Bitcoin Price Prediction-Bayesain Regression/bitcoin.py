...
def computeDelta(wt, X, Xi):
    ...
    """
    # YOUR CODE GOES HERE
    predict_numerator = 0.0
    predict_denominator = 0.0

    for i in range(len(Xi)):
       xi = Xi.iloc[i][0:len(Xi.iloc[0]) - 1]
       yi = Xi.iloc[i][len(Xi.iloc[0]) - 1]
       #print yi
       #print xi
       ...
       stdx = np.std(x)
       stdxi = np.std(xi)
       #print meanx,meanxi,stdx,stdxi

       similarity = 0.0
       for i in range(len(x)):
            similarity += (x[i] - meanx) * (xi[i] - meanxi)
       similarity = similarity / (len(x) * stdx * stdxi)
       #print "Similarity: %f" %similarity

       predict_numerator += yi * math.exp(wt * similarity)
       predict_denominator += math.exp(wt * similarity)
    
    #print "predict_numerator: %f" %predict_numerator
    #print "predict_denominator: %f" %predict_denominator
